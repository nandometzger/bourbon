        ...
        if is_ensemble:
             ...
             with torch.no_grad():
                 for i in range(input_tensor.shape[0]):
                     ...
                     # 1. Filter if empty/masked (High Threshold: 60%)
                     raw_slice = image[i]
                     nz = np.count_nonzero(raw_slice) / raw_slice.size
                     if nz < 0.6: continue
                     
                     # 2. Patch-level Cloud Filter
                     # Clouds are bright in the Blue band (index 2 in [R,G,B,N]).
                     # Aggressive: If > 20% of pixels exceed 2200 reflectance, reject.
